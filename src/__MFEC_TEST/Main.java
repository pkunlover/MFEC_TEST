	...
	public static void main(String[] args) throws FileNotFoundException, ParseException {
		...
		for (int i = 0; i < arlIn.size(); i++) {
			if (!arlIn.get(i).getStartTime().equals("00.00")) {
				...
				try {
					...
					if (dS.getTime() > dOT.getTime()) {
						ot_Min = dE.getTime() - dS.getTime();
					} else {
						ot_Min = dE.getTime() - dOT.getTime();
					}
					...
					arlSa.add(new Salaries(arlIn.get(i).getName(), nol_Min, ot_Min, late_Min, dS.getDay()));
...

		{
			...
			for (int j = i + 0; j < arlIn.size(); j += hNum) {
				if (arlSa.get(i).getName().equals(arlSa.get(j).getName())) {
					...
					tmpOtSal += arlSa.get(j).getOtSalaries();
					tmpLateSal += arlSa.get(j).getLateSalaries();
				}
			}
...

		{
			...
			System.out.print(arlSaTotal.get(i).getName() + "  \t\t" + dcf.format(arlSaTotal.get(i).getNolSalaries())
					...
					+ dcf.format(arlSaTotal.get(i).getLateSalaries()) + "     \t"
					+ dcf.format(arlSaTotal.get(i).getTotalSalaries()));
			System.out.println("     \t|");
		}
		System.out.printf(
				"|===============================================================================================|\n");
